...
class LiquidationManuelleScreen extends React.Component {
  ...
  loadListeTaxesArticle(refLignesRubriqueBaseLiquidation) {
    refLignesRubriqueBaseLiquidation.map((item, key) => {
      ...
      this.setState({
        listeTaxesArticle: [...this.state.listeTaxesArticle, item],
      });
  ...
  confirmer = async () => {
    ...
    if (
      ...
      await callLiquidationUpdateRedux(this.props, {
        command: this.state.commande,
        typeService: 'UC',
        jsonVO: data,
      });
  ...
  liquiderConsignerArticleManuelle = async () => {
    ...
    var refLignesRubriqueBaseLiquidation = this.state.listeTaxesArticle.map(
      function (item) {
        return {
    ...
    if (!article.refArticleLiquideReference) {
      if (this.props.liquidationType == 'automatique') {
        ...
        normalContext =
          !(
            article.refParametresLiquidation.indicateurConsignationIntegrale ==
            'true'
          ) + '';
        consignationContext =
          (article.refParametresLiquidation.indicateurConsignationIntegrale ==
    ...
    data = {
      indicateurLiquidationManuelleRequise:
        article.indicateurLiquidationManuelleRequise,
      ...
      refOperationDeBase:
        article.refParametresLiquidation.indicateurConsignationIntegrale ==
        'true'
          ? this.state.liquidationVO.idOperation
          : article.refOperationDeBase,
      refParametresLiquidation: {
        idParameterLiquidation:
          article.refParametresLiquidation.idParameterLiquidation,
        codeNomenclature: article.refParametresLiquidation.codeNomenclature,
        dateEffetLiquidation:
          article.refParametresLiquidation.dateEffetLiquidation,
        designation: article.refParametresLiquidation.designation
          ? article.refParametresLiquidation.designation
          : '.',
        indicateurConsignationIntegrale:
          article.refParametresLiquidation.indicateurConsignationIntegrale,
        indicateurFranchiseTotale:
          article.refParametresLiquidation.indicateurFranchiseTotale,
    ...
    if (commande == 'confirmerConsignerArticle') {
      ...
      data.refArticleLiquideReference = {
        idArticleLiquideParOperation:
          article.refArticleLiquideReference.idArticleLiquideParOperation,
      };
    }
                  ...
                  <Row style={CustomStyleSheet.whiteRow}>
                    ...
                    <Col size={0.4}>
                      <Checkbox
                        color={'#009ab2'}
                        status={
                          ligneRubrique.indicateurTVA ? 'checked' : 'unchecked'
                        }
                        disabled={false}
                        onPress={() =>
                          this.setState({
                            ...
                            ligneRubrique: {
                              ...this.state.ligneRubrique,
                              indicateurTVA: !this.state.ligneRubrique
                                .indicateurTVA,
                            },
                          })
                        }
                    ...
                    <Col size={0.4}>
                      <Checkbox
                        color={'#009ab2'}
                        status={
                          ligneRubrique.indicateurFranchise
                            ? 'checked'
                            : 'unchecked'
                        }
                        disabled={false}
                        onPress={() =>
                          this.setState({
                            ...
                            ligneRubrique: {
                              ...this.state.ligneRubrique,
                              indicateurFranchise: !this.state.ligneRubrique
                                .indicateurFranchise,
                            },
                          })
                        }
