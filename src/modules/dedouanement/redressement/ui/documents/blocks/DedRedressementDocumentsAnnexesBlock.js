...
class DedRedressementDocumentsExigiblesBlock extends React.Component {
  ...
  constructor(props) {
    super(props);
    this.state = {
      documentsAnnexesCols: this.documentsAnnexesCols(),
  ...
  render() {
    let documentsAnnexes = this.extractCommandData(
      'ded.recupererDocumentsAnnexesDUM',
      'genericDedReducer',
    );
    ...
    return (
      <View style={styles.container}>
        {/*Accordion Documents documentsAnnexes*/}
        <ComAccordionComp
          title={translate('dedouanement.documents.documentsAnnexes')}
          expanded={true}>
          {!_.isNil(documentsAnnexes) && !_.isNil(documentsAnnexes.data) && (
            <ComBasicDataTableComp
              hasId={true}
              libelleIdCol={translate('dedouanement.documents.num')}
              rows={documentsAnnexes.data}
              cols={this.state.documentsAnnexesCols}
              totalElements={
                documentsAnnexes.data ? documentsAnnexes.data.length : 0
              }
              maxResultsPerPage={5}
              paginate={true}
            />
