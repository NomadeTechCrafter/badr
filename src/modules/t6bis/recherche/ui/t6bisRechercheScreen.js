...
import { ComBadrErrorMessageComp, ComBadrToolbarComp } from '../../../../commons/component';
...
class T6bisRecherche extends React.Component {
    ...
    valider = () => {
        ...
        if (this.state.mode === 'redressement') {
            this.state.t6bis.referenceEnregistrement = this.state.bureau + "" + this.state.annee + "" + this.state.serie;
        } else if (!this.state.t6bis.enregistree) {
            console.log("1", this.state.t6bis.enregistree);
            this.state.t6bis.referenceProvisoire = this.state.bureau + "" + this.state.annee + "" + this.state.serie;
        } else {
        ...
        return (
            <ScrollView>
                <View style={styles.container}>
                    ...
                    {(!_.isEmpty(this.state.bureau) && !_.isEmpty(this.state.annee) && !_.isEmpty(this.state.serie)) && (<View style={styles.enregistreeStyle}>
                        <Checkbox.Item
                            status={this.state.t6bis.enregistree ? 'checked' : 'unchecked'}
                            label={translate('t6bisrecherche.fields.enregistree')}
                            color={primaryColor}
                            onPress={() => {
                                this.setState({ t6bis: { ...this.state.t6bis, enregistree: !this.state.t6bis.enregistree } });
                                this.completeTextInputFields();
                    ...
                    {(!_.isEmpty(this.state.bureau) && !_.isEmpty(this.state.annee) && !_.isEmpty(this.state.serie)) && (<View style={styles.ComContainerCompBtn}>
                        <Button
