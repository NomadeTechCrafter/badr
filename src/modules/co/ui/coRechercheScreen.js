...
class coRechercheScreen extends React.Component {
  constructor(props) {
    super(props);
  ...
  handleSearch = () => {
    if (this.state.codeLieuChargement || this.state.referenceLot) {
      if (
        this.state.codeLieuChargement &&
        this.state.referenceLot &&
        this.state.referenceDS
      ) {
        let action = this.buildSearchCOConsultationAction(this.state.login);
        this.props.actions.dispatch(action);
      } else {
        this.setState({
          errorMessage:
            "Merci de saisire les 3 criteres de recherche d'un lot(Réference DS/ Réference Lot / Lieu de Chargement).",
        });
      }
    } else {
      let action = this.buildSearchCOConsultationAction(this.state.login);
  ...
  buildSearchCOConsultationAction = () => {
    let action = request({
        ...
        <View style={CustomStyleSheet.verticalContainer20}>
          ...
          <View style={CustomStyleSheet.row}>
            <TextInput
              ...
              mode="outlined"
              label={translate('Cco.filtreRecherche.matricule')}
          ...
          <View style={CustomStyleSheet.row}>
            <TextInput
              style={CustomStyleSheet.column}
              mode="outlined"
              label={translate('co.filtreRecherche.refDS')}
            ...
            <TextInput
              style={CustomStyleSheet.column}
              mode="outlined"
              label={translate('co.filtreRecherche.refLot')}
          ...
          <View style={CustomStyleSheet.row}>
            <ComBadrAutoCompleteChipsComp
              code="code"
              placeholder={translate('co.filtreRecherche.lieuChargement')}
...
export default connect(mapStateToProps, mapDispatchToProps)(coRechercheScreen);
