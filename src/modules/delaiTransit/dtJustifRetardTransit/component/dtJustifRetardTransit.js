...
import * as InitJustifRetardTransitAction from '../state/actions/dtInitJustifRetardTransitAction';
import { MODULE_ECI ,MODULE_ECOREXP} from '../../../../commons/Config';

class DtJustifRetardTransit extends Component {
  defaultState = {
    bureau: '',
    regime: '',
    annee: '',
  ...
  componentDidMount() {
    var action = InitJustifRetardTransitAction.init({
      type: Constants.INIT_JUSTIF_RETARD_TRANSIT_INIT,
      value: {},
    });
  ...
  confirmer = () => {
    ...
    if (this.state.regime && this.state.serie) {
      ...
      if (this.state.cle === this.state.cleValide) {
        ...
        let dataAction = {
          type: Constants.INIT_JUSTIF_RETARD_TRANSIT_REQUEST,
          value: {
        ...
        let action = InitJustifRetardTransitAction.request(dataAction);
...
const mapStateToProps = (state) => ({
  ...state.justifRetardTransitReducer,
});


export default connect(mapStateToProps, null)(DtJustifRetardTransit);
