...
import * as AnnoterInitAction from '../../state/actions/decAnnoterInitAction';

class DecAnnoterRechercheTryptique extends Component {
  defaultState = {
    bureau: '',
    regime: '',
    annee: '',
  ...
  componentDidMount() {
    var action = AnnoterInitAction.init();
  ...
  confirmer = () => {
    ...
    if (this.state.regime && this.state.serie) {
      ...
      if (this.state.cle === this.state.cleValide) {
        ...
        var action = AnnoterInitAction.request(
          {
            type: Constants.ANNOTER_RECHERCHE_D17_DUM_REQUEST,
            value: {
...
const mapStateToProps = (state) => ({...state.annoterInitReducer});
