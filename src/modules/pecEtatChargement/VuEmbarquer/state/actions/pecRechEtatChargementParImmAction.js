            ...
            .then((response) => {
                if (response) {
                    ...
                    if (
                        ...
                        console.log('++++++++++++++++++++++++++++++++++++++++++   ', JSON.stringify(data?.jsonVO?.refEtatChargement));
                        let refEtatChargement = data?.jsonVO?.refEtatChargement;
                        let actionInitVuEmbarquer = pecInitVuEmbarquer.request(
                            {
                                type: INIT_ETAT_CHARGEMENT_VE_REQUEST,
                                value: {
                                    codeBureau: refEtatChargement?.refBureauDouane?.codeBureau,
                                    regime: refEtatChargement.regime,
                                    anneeEnregistrement: refEtatChargement.anneeEnregistrement,
                                    numeroSerieEnregistrement: refEtatChargement.numeroSerieEnregistrement,
                                    cleIHM: refEtatChargement.cle,
