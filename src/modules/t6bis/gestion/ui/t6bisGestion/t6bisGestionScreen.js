...
function ArticlesTab({route, navigation}) {
  return <T6bisArticlesTab navigation={navigation} route={route} />;
}

function TaxationManuelleTab({route, navigation}) {
  return <T6bisTaxationManuelleTab navigation={navigation} route={route} />;
}
function TaxationGlobaleTab({route, navigation}) {
  return <T6bisTaxationGlobaleTab navigation={navigation} route={route} />;
}
function InformationsTab({route, navigation}) {
  return <T6bisInformationsTab navigation={navigation} route={route} />;
}
function HistoriqueTab({route, navigation}) {
  return <T6bisHistoriqueTab navigation={navigation} route={route} />;
}
...
class T6bisGestion extends React.Component {
  ...
  lancerUC = (uc) => {
    if (
      isCreation() &&
      this.props.t6bis &&
      this.props.t6bis.dateEnregistrement
    ) {
      ...
      if (
        isCreation &&
        this.props.t6bis &&
        this.props.t6bis?.codeTypeT6bis === '01'
      ) {
  ...
  suppressionEffective = () => {
    ...
    if (!result) {
      ...
      this.hideDialog();
      console.log(
        '--------------------------------------------------SuppressionEffective---  4  -------------------------------------------------------------------------',
      );
    } else {
      console.log(
        '--------------------------------------------------SuppressionEffective---  5 2 -------------------------------------------------------------------------',
      );
      ...
      <ScrollView style={styles.container}>
        ...
        <View style={{flex: 1}}>
          {!isMtm(codeTypeT6bis) && !isCm(codeTypeT6bis) && (
            <Tab.Navigator
              ...
              <Tab.Screen
                name={translate('t6bisGestion.tabs.entete.title')}
                component={EnteteTab}
              />
              <Tab.Screen
                name={translate('t6bisGestion.tabs.taxation.globale.title')}
                component={TaxationGlobaleTab}
              />
              <Tab.Screen
                name={translate('t6bisGestion.tabs.informations')}
                component={InformationsTab}
              />
              <Tab.Screen
                name={translate('t6bisGestion.tabs.historique.title')}
          ...
          {(isMtm(codeTypeT6bis) || isCm(codeTypeT6bis)) && (
            <Tab.Navigator
              ...
              />
              <Tab.Screen
                name={translate('t6bisGestion.tabs.articles.title')}
                component={ArticlesTab}
              />
              <Tab.Screen
                name={translate('t6bisGestion.tabs.taxation.manuelle.title')}
                component={TaxationManuelleTab}
              />
        ...
        <ComBadrDialogComp
          ...
          confirmMessage={translate(
            't6bisGestion.suppression.confirmDialog.oui',
          )}
          cancelMessage={translate(
            't6bisGestion.suppression.confirmDialog.non',
          )}
          dialogMessage={translate(
            't6bisGestion.suppression.confirmDialog.suppressionMessage',
          )}
...
export default connect(mapStateToProps, mapDispatchToProps)(T6bisGestion);
