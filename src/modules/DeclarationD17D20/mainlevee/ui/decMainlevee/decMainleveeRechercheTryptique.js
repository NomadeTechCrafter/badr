...
import * as MainleveeInitAction from '../../state/actions/decMainleveeInitAction';
...
class MainleveeRechercheTryptique extends Component {
  ...
  componentDidMount() {
    var action = MainleveeInitAction.init();
    this.props.actions.dispatch(action);
    // var action1 = MainleveeScanAction.init();
  ...
  confirmer = () => {
    ...
    if (this.state.regime && this.state.serie) {
      ...
      if (this.state.cle === this.state.cleValide) {
        ...
        var action = MainleveeInitAction.request(
          {
            type: Constants.RECHERCHE_D17_MAINLEVEE_REQUEST,
            value: {
...
const mapStateToProps = (state) => ({ ...state.mainleveeInitReducer });
