...
export function request(action) {
  return (dispatch) => {
    ...
    EciApposerScellesApi.getScellesDeclarees(action.value.referenceEnregistrement, action.value.numeroVoyage)
      .then((response) => {
        if (response) {
...
export function inProgress(action) {
  return {
    type: Constants.ECI_GET_SCELLES_DECLAREES_IN_PROGRESS,
    value: action.value,
  };
}
...
export function success(data) {
  return {
    type: Constants.ECI_GET_SCELLES_DECLAREES_SUCCESS,
    value: {
      data: data
    },
  };
...
export function failed(data) {
  return {
    type: Constants.ECI_GET_SCELLES_DECLAREES_FAILED,
    value: data,
  };
}
