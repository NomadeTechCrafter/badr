...
export function request(action, navigation,screen) {
    return (dispatch) => {
        ...
        T6bisGestiontionApi.sauvegarderT6BisTPE(action.value.cmd, action.value.t6bis)
...
export function failed(action) {
    return {
        type: Constants.T6BIS_SAUVEGARDER_TPE_FAILD,
        value: action.value,
    };
}
...
export function inProgress(action) {
    return {
        type: Constants.T6BIS_SAUVEGARDER_TPE_PROGRESS,
        value: action.value,
    };
}
