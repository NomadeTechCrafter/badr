...
import style from '../../../vuEmbarquer/style/vuEmbarquerStyle';
...
class RechParRefConnaissements extends React.Component {
  defaultState = {
    selectedDum: {},
  };
  ...
  constructor(props) {
    ...
    this.cols = [
      {
        code: 'vo.numeroOrdre',
        libelle: translate('sortiPort.numeroVersion'),
        width: 200,
      },
      {
        code: 'vo.typeDS',
  ...
  render() {
    let preapurements = this.props.dataVo?.trypRecherchePreapSectionVO
      ?.dedDumPreapWrappers;
    if (!preapurements) {
      preapurements = [];
    }
        ...
        <ScrollView>
          ...
          <CardBox style={style.cardBox}>
            <Accordion
              badr
              title={
                'Nombre total des préapurements : ' + preapurements.length
              }>
              {/* {resultatsScanner && ( */}
              <ComBasicDataTableComp
                ref="_badrTable"
