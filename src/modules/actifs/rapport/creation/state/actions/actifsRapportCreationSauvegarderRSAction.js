    ...
    TransverseApi.doProcess(
      ...
      .then((response) => {
        if (response) {
          ...
          console.log('response sauvegarderRS', JSON.stringify(response));
          if (data && !data.dtoHeader.messagesErreur) {
            console.log('data sauvegarderRS', data);
            dispatch(success(data));
            /** Naviguer vers la vue suivant. */

            console.log('navig success');
          } else {
            console.log('failed(data) sauvegarderRS',data);
            dispatch(failed(data));
          }
        } else {
          console.log('failed(data) 2 sauvegarderRS', response);
          dispatch(failed(translate('errors.technicalIssue')));
        }
      })
      .catch((e) => {
        console.log('in action request catch sauvegarderRS ', e);
        dispatch(failed(translate('errors.technicalIssue')));
      });
  };
}
