'use strict';
import React from 'react';
import {Text, View} from 'react-native';
/** redux */
import {connect} from 'react-redux';
import * as Constants from '../../../constants/components/ComQrCodeConstants';
import * as QrCodeAction from '../../../state/actions/ComQrCodeAction';

import * as Zxing from '../../../native/ComZxingNative';

class ComQrCodeScannerComp extends React.Component {
  constructor(props) {
    super(props);
    Zxing.default.showQrReader(this.onBarcodeRead);
  }

  onBarcodeRead = (data) => {
    if (data) {
      let action = QrCodeAction.request({
        type: Constants.QRCODE_REQUEST,
        value: {
          module: 'DED_LIB',
          command: 'ded.lireCodeQr',
          typeService: 'SP',
          param: data,
        },
      });
      this.props.dispatch(action);
    }
    this.props.navigation.navigate(this.props.route.params.screenAfterScan, {});
  };

  render() {
    return (
      <View>
        <Text>Merci de patienter un moment...</Text>
      </View>
    );
  }
}

const mapStateToProps = (state) => {
  return {...state.qrCodeReducer};
};

export default connect(mapStateToProps, null)(ComQrCodeScannerComp);
