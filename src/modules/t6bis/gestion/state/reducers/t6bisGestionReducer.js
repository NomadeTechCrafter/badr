...
import {
  calculateTotalT6bis,
  getCurrentArticle,
  groupLignesByRubrique,
  groupLignesByRubriqueByArticle,
  hasAtLeastOneTaxationLine,
  isCm,
  isCreation,
  isMtm,
...
export default (state = initialState, action) => {
  ...
  switch (action.type) {
    ...
    case Constants.T6BIS_INIT_ENTETE_REQUEST:
      console.log(
        '11/02/2022  T6BIS_INIT_ENTETE_REQUEST ',
        Constants.T6BIS_INIT_ENTETE_REQUEST,
      );
    ...
    case Constants.T6BIS_INIT_ENTETE_SUCCES:
      console.log(
        '11/02/2022  T6BIS_INIT_ENTETE_SUCCES ',
        Constants.T6BIS_INIT_ENTETE_SUCCES,
      );
      ...
      if (action.value.mode === MODE_CREATION) {
        ...
        nextState.currentArticle = getCurrentArticle(
          nextState.t6bis.codeTypeT6bis,
          0,
        );
        ...
        if (
          isMtm(action.value.t6bis.codeTypeT6bis) ||
          isCm(action.value.t6bis.codeTypeT6bis)
        ) {
          if (
            nextState.t6bis.listeArticleT6bis &&
            nextState.t6bis.listeArticleT6bis.length >= 1
          ) {
            ...
            nextState.montantGlobalByArticle = calculateTotalT6bis(
              recapCurrentArticleList,
              nextState.t6bis,
            );
        ...
        nextState.t6bis.montantGlobal = calculateTotalT6bis(
          listeRecap,
          nextState.t6bis,
        );
    ...
    case Constants.T6BIS_UPDATE_INTERVENANT_SUCCES:
      nextState.t6bis.intervenantVO.nomIntervenant =
        action.value?.intervenantVO?.nomIntervenant;
      nextState.t6bis.intervenantVO.prenomIntervenant =
        action.value?.intervenantVO?.prenomIntervenant;
      nextState.t6bis.intervenantVO.adresse =
        action.value?.intervenantVO?.adresse;
      nextState.t6bis.intervenantVO.refPaysPassPort =
        action.value?.intervenantVO?.refPaysPassPort;
      return nextState;
    ...
    case Constants.T6BIS_ADD_TAXATION_ARTICLE_SUCCES:
      ...
      nextState.currentArticle.montantGlobalByArticle = calculateTotalT6bis(
        recapCurrentArticleList,
        nextState.t6bis,
      );
      nextState.recapCurrentArticleList = recapCurrentArticleList;
    ...
    case Constants.T6BIS_SAUVEGARDER_SUCCES:
      ...
      nextState.errorMessage = action.value.dtoHeader
        ...
        : action.value;
      console.log(
        '11/02/2022 action.value.cmd == CMD_ENREGISTRER_T6BIS ',
        action.value.cmd == CMD_ENREGISTRER_T6BIS,
      );
      console.log('11/02/2022 action.value.cmd ', action.value.cmd);
      if (
        action.value.cmd ==
        CMD_ENREGISTRER_T6BIS + nextState.t6bis.codeTypeT6bis
      ) {
    ...
    case Constants.T6BIS_ENREGISTRER_TPE_FAILD:
      nextState.successMessage = null;
      nextState.showProgress = false;
      nextState.errorMessage = action.dtoHeader
        ? action.dtoHeader.messagesErreur
        : null;
      //   console.log('failenregi',JSON.stringify(action));
      return nextState;
    ...
    case Constants.T6BIS_ENREGISTRER_TPE_SUCCES:
      ...
      nextState.errorMessage = action.value.dtoHeader
        ? action.value.dtoHeader.messagesErreur
        : null;
      //   console.log('succesnregi',JSON.stringify(nextState.errorMessage));
      return nextState;
...
const startRedevableCompletion = function (t6bis, redevableResponse) {
  ...
  t6bis.intervenantVO.numeroOrdreIntervenant =
    redevableResponse.numeroOrdreIntervenant;
  t6bis.intervenantVO.refPaysPassPort = redevableResponse.refPaysPassPort;
  t6bis.intervenantVO.refTypeDocumentIdentite =
    redevableResponse.identifiants.typeIdentifiant;
  t6bis.intervenantVO.numeroDocumentIndentite =
    redevableResponse.identifiants.numeroDocumentIdentite;
};
...
const clearRedevableCompletion = function (t6bis, values) {
  ...
  t6bis.intervenantVO.refTypeDocumentIdentite =
    values.identifiants.typeIdentifiant;
  t6bis.intervenantVO.numeroDocumentIndentite =
    values.identifiants.numeroDocumentIdentite;
};
