...
class LiqRecapitulationLiqNormaleInitialeBlock extends React.Component {
  ...
  componentDidMount() {
    ...
    this.setState({
      ...
      dateEnregistrement: this.props.liquidationVO?.refObjetLiquidation
        ?.dateEnregistrement,
    });
  }
  ...
  showDetailArticle = (article, libelleArticle) => {
    ...
    if (
      this.props.liquidationType == 'manuelle' ||
      this.props.liquidationType == 'manuelleOffice' ||
      this.props.liquidationType == 'manuelleRedevanceAT'
    ) {
      this.setState({
        articleEnCours: {
  ...
  checkRequiredFieldsResultatCtrl = (params) => {
    ...
    if (_.isEmpty(modele.refUniteQuantite)) {
      ...
      params.msg +=
        '- ' +
        translate('liq.articles.refUniteQuantite') +
        ' ' +
        translate('liq.error.shouldNotBeEmpty');
    }
    ...
    if (_.isEmpty(modele.valeurTaxable)) {
      ...
      params.msg +=
        '- ' +
        translate('liq.articles.valeurTaxable') +
        ' ' +
        translate('liq.error.shouldNotBeEmpty');
    }
  ...
  confirmerArticle = () => {
    ...
    if (!this.checkRequiredFields()) {
      let articleALiquider = {
        ...
        consignationContext: articleEnCours.consignationIntegrale
          ? 'true'
          : 'false',
        refParametresLiquidation: {
          indicateurConsignationIntegrale: articleEnCours.consignationIntegrale
            ? 'true'
            : 'false',
          codeNomenclature: articleEnCours.codeNomenclature,
          dateEffetLiquidation: articleEnCours.dateEffetLiquidation
            ? articleEnCours.dateEffetLiquidation
            : this.props.liquidationVO?.refObjetLiquidation?.dateEnregistrement,
          designation: articleEnCours.designation,
          indicateurFranchiseTotale: articleEnCours.franchiseTotale
            ? 'true'
            : 'false',
      ...
      if (actionType == 'add') {
        ...
        if (articleEnCours.consignationIntegrale) {
          if (
            ...
            articleALiquider.refOperationLiquidation =
        ...
        console.log(
          '----LIQ articleALiquider - add : ',
          JSON.stringify(articleALiquider),
        );
        ...
        if (selectedArticle.refArticleLiquideReference) {
          ...
          articleALiquider.idArticleLiquideParOperation =
            selectedArticle.idArticleLiquideParOperation;
          articleALiquider.consignationContext = 'false';
          articleALiquider.normalContext = 'false';
          articleALiquider.refParametresLiquidation.idParameterLiquidation =
            selectedArticle.refParametresLiquidation.idParameterLiquidation;
          articleALiquider.refArticleLiquideReference = {
            idArticleLiquideParOperation:
          ...
          if (selectedArticle.liquidationFinsConsignation) {
            ...
            articleALiquider.idArticleLiquideParOperation =
              selectedArticle.idArticleLiquideParOperation;
            if (
              articleALiquider.refParametresLiquidation
                .indicateurConsignationIntegrale == 'true'
            ) {
              ...
              articleALiquider.refOperationLiquidation =
                liquidationVO.idOperation;
            }
            articleALiquider.refOperationDeBase = liquidationVO.idOperation;
          }
        }
        console.log(
          '----LIQ articleALiquider - edit : ',
          JSON.stringify(articleALiquider),
        );
        callRedux(this.props, {
          command: commande,
      ...
      console.log(
        '----LIQ Action response  - ',
        JSON.stringify(this.props.liquidationReducer),
      );
  ...
  render() {
    ...
    console.log(
      'liquidationType ======>',
      liquidationType + ' ' + this.state.compteur++,
    );
    const {
      listeArticlesLiquides,
      selectedArticle,
            ...
            <Grid>
              <Row style={CustomStyleSheet.lightBlueRow}>
                <Checkbox
                  color={'#009ab2'}
                  status={
                    this.state.articleEnCours.franchiseTotale
                      ? 'checked'
                      : 'unchecked'
                  }
                  disabled={false}
                  onPress={() =>
                    this.setState({
                      articleEnCours: {
                        ...this.state.articleEnCours,
                        franchiseTotale: !this.state.articleEnCours
                          .franchiseTotale,
                      },
                    })
                  }
              ...
              <Row style={CustomStyleSheet.lightBlueRow}>
                <Checkbox
                  color={'#009ab2'}
                  status={
                    this.state.articleEnCours.consignationIntegrale
                      ? 'checked'
                      : 'unchecked'
                  }
                  disabled={false}
                  onPress={() =>
                    this.setState({
                      articleEnCours: {
                        ...this.state.articleEnCours,
                        consignationIntegrale: !this.state.articleEnCours
                          .consignationIntegrale,
                      },
                    })
                  }
              ...
              <Row style={CustomStyleSheet.whiteRow}>
                ...
                <Col size={1}>
                  <ComBadrAutoCompleteChipsComp
                    onRef={(ref) =>
                      (this.state.articleEnCours.refUniteQuantite = ref)
                    }
                    code="code"
                ...
                {libelleArticle.toLowerCase().includes('consignation') && (
                  <Row style={CustomStyleSheet.whiteRow}>
                    ...
                    <Col>
                      <ComBadrLibelleComp>
                        {': '}
                        {Numeral(selectedArticle.montantConsigne).format(
                          '0.00',
                        )}
                      </ComBadrLibelleComp>
                    </Col>
